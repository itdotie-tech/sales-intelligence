/* Central store for manual corrections (account managers, name links, renames).
   ...
   Node's built-in fetch + crypto (Shared Key auth). This means the API has
   nothing to npm-install, so it cannot fail to build on a missing package.
   getSource does NOT import this file, so even if storage is misconfigured the
   Nimble feed is unaffected — corrections simply fall back to per-browser. */
...
function authHeader(c, method, path, query, headers, bodyLen){
  const canonHeaders = Object.keys(headers).filter(k=>k.startsWith('x-ms-')).sort()
    .map(k=>`${k}:${headers[k]}`).join('\n');
  const qk = Object.keys(query).sort();
  const canonResource = `/${c.account}/${path}` + (qk.length ? '\n'+qk.map(k=>`${k}:${query[k]}`).join('\n') : '');
  const clen = bodyLen>0 ? String(bodyLen) : '';
  const stringToSign = [
    method, '', '', clen, '', headers['content-type']||'', '', '', '', '', '', ''
  ].join('\n') + '\n' + canonHeaders + '\n' + canonResource;
...
async function ensureContainer(){
  const r = await request('PUT', CONTAINER, { restype:'container' }, {}, null);
  if(!(r.status===201 || r.status===409)) { /* 409 = already exists; anything else is surfaced on read/write */ }
}
